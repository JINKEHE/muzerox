...
def muzero_policy(
  ...
  # extra data - min and max qvalues
  
  extra_data = action_selection.MuZeroExtraData(
    min_value=jnp.ones((root.prior_logits.shape[0], ), dtype=jnp.float32)*(1000),
    max_value=jnp.ones((root.prior_logits.shape[0], ), dtype=jnp.float32)*(-1000)
  )
