...
def switching_action_selection_wrapper(
  ...
  def switching_action_selection_fn(
      ...
      depth: base.Depth,
      max_value,
      min_value) -> chex.Array:
    return jax.lax.cond(
        depth == 0,
        lambda x: root_action_selection_fn(*x[:3], max_value=max_value, min_value=min_value),
        lambda x: interior_action_selection_fn(*x, max_value=max_value, min_value=min_value),
...
def muzero_action_selection(
  ...
  max_value = jnp.maximum(max_value, jnp.max(safe_qvalues))
  min_value = jnp.minimum(min_value, jnp.min(safe_qvalues))
  # check if qtransform is a partial function based on qtransforms.qtransform_by_updatable_min_max
  if hasattr(qtransform, 'func') and qtransform.func == qtransforms.qtransform_by_updatable_min_max:
    value_score = qtransform(tree, node_index, min_value=min_value, max_value=max_value)
  else:
  ...
  return max_value, min_value, masked_argmax(to_argmax, tree.root_invalid_actions * (depth == 0))

@chex.dataclass(frozen=True)
class MuZeroExtraData:
  """Extra data for MuZero search."""
  min_value: chex.Array
  max_value: chex.Array
