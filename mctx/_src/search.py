...
def search(
  ...
  def body_fun(sim, loop_state):
    ...
    tree, parent_index, action = simulate(simulate_keys, tree, action_selection_fn, max_depth)
...
def simulate(
    ...
    max_depth: int) -> Tuple[tree_lib.Tree, chex.Array, chex.Array]:
  ...
  def body_fun(state):
    ...
    max_value, min_value, action = action_selection_fn(action_selection_key, tree, node_index, state.depth, state.max_value, state.min_value)
  ...
  initial_state = _SimulationState(
      ...
      max_value=tree.extra_data.max_value,
      min_value=tree.extra_data.min_value)
  ...
  tree = tree.replace(
    extra_data=tree.extra_data.replace(
      max_value=end_state.max_value,
      min_value=end_state.min_value
    )
  )
  return tree, end_state.node_index, end_state.action
